...
export default function MenuDesktop({ isOffset, isHome, navConfig }) {
	...
	return (
		<Stack direction='row'>
			{navConfig.map((link) => (
				<MenuDesktopItem key={link.title} item={link} isOffset={isOffset} isHome={isHome} pathname={pathname} />
			))}
		</Stack>
	)
}
...
function MenuDesktopItem({ item, isHome, isOffset, pathname }) {
				...
				>
					<Grid container spacing={3}>
						{children.map((list) => {
							...
							return (
								<Grid key={subheader} item xs={12} md={subheader === 'Dashboard' ? 2 : 10}>
									<List disablePadding>
										<ListSubheader
											disableSticky
											disableGutters
